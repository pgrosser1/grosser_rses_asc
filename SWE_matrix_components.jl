...
@kernel function Auu!(Auuφ, grid, φ)
    ...
    @inbounds Auuφ[i, j, k] = drag_coefficient * (1/H) * φ[i, j, k]
end
...
@kernel function Auv!(Auvφ, grid, φ)
    ...
    depth = grid.immersed_boundary.bottom_height[i, j]
    H = ifelse(depth<0, -depth, 0)
    @inbounds Auvφ[i, j, k] = - f * (1/H) * ℑxyᶠᶜᵃ(i, j, k, grid, φ)
...
@kernel function Avu!(Avuφ, grid, φ)
    ...
    depth = grid.immersed_boundary.bottom_height[i, j]
    H = ifelse(depth<0, -depth, 0)
    @inbounds Avuφ[i, j, k] = -f * (1/H) * ℑxyᶜᶠᵃ(i, j, k, grid, φ)
end
...
@kernel function Avv!(Avvφ, grid, φ)
    ...
    depth = grid.immersed_boundary.bottom_height[i, j]
    H = ifelse(depth<0, -depth, 0)
    @inbounds Avvφ[i, j, k] = drag_coefficient * (1/H) * φ[i, j, k]
end
...
@kernel function Aηu!(Aηuφ, grid, φ)
    i, j, k = @index(Global, NTuple)
    @inbounds Aηuφ[i, j, k] = (1-β) * ∂xᶜᶜᶜ(i, j, k, grid, φ) # H = depth of ocean
end

@kernel function Aηv!(Aηvφ, grid, φ)
    i, j, k = @index(Global, NTuple)
    @inbounds Aηvφ[i, j, k] = (1-β) * ∂yᶜᶜᶜ(i, j, k, grid, φ)
end
